...
def find_data_dir():
    for pattern in ['kaggle_data/**/*.csv', 'kaggle_data/*.csv']:
...
def main():
    ...
    if not data_dir:
        ...
        for root, dirs, files in os.walk("kaggle_data"):
            for f in files[:10]: print(f"  {os.path.join(root, f)}")
        sys.exit(1)
    ...
    for f in sorted(data_path.glob("price_full_file_*.csv")):
        ...
        print(f"  {chain_name} ({fsize:.1f}MB, {lines} rows)...", flush=True)
        t0 = time.time()
        ...
        if time.time() - start > 5400:
            print("  WARNING: Approaching time limit, stopping"); break
    print("\n=== Updating stats ===")
