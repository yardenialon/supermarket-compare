...
function ProductImg({ barcode, name, size = 48, imageUrl, clickable = true }: { barcode: string; name: string; size?: number; imageUrl?: string | null; clickable?: boolean }) {
  ...
  const proxyUrl = imageUrl && !err
    ? 'https://supermarket-compare-production.up.railway.app/api/image-proxy?u=' + Buffer.from(imageUrl).toString('base64')
    : '';
  const url = proxyUrl;
...
export default function Home() {
  ...
  const { isOpen: scannerOpen, open: openScanner, close: closeScanner, handleScan } = useBarcodeScanner();
  ...
  return (
    ...
    <div className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-xl flex items-center justify-center" style={{background:'linear-gradient(135deg,#10b981,#059669)'}}>
          ...
          <div className="space-y-2">
            ...
            {sorted.map((p: Product) => (
              <div key={p.id} className={"group rounded-xl transition-all bg-white border " + (sel?.id === p.id ? "border-emerald-500 shadow-md ring-1 ring-emerald-500/20" : "border-stone-100 hover:border-stone-200 hover:shadow-sm")}>
                ...
                <div className="px-4 pb-3 -mt-1 flex items-center gap-2"><button onClick={() => addToList(p)} className="text-sm sm:text-xs px-5 sm:px-4 py-2 sm:py-1.5 rounded-lg bg-emerald-50 text-emerald-600 font-semibold hover:bg-emerald-100 transition">+ לרשימה</button><a href={`/product/${p.id}`} className="text-sm sm:text-xs px-4 sm:px-3 py-2 sm:py-1.5 rounded-lg bg-stone-50 text-stone-400 font-semibold hover:bg-stone-100 hover:text-stone-600 transition flex items-center gap-1">דף מוצר <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg></a></div>
              </div>
            ))}
            {!loading && !q.trim() && (
      ...
      {scannerOpen && <BarcodeScanner onScan={handleScan} onClose={closeScanner} />}
<LogoMarquee />
