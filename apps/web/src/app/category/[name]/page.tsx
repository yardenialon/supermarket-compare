          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {products.map((p) => (
              <Link key={p.id} href={`/product/${p.id}`}
                ...
                {p.minPrice && (
                  <div className="font-mono font-black text-emerald-600 text-lg">₪{Number(p.minPrice).toFixed(2)}</div>
                )}
                {p.storeCount && (
                  <div className="text-xs text-stone-400">{p.storeCount} חנויות</div>
                )}
              </Link>
            ))}
          </div>
