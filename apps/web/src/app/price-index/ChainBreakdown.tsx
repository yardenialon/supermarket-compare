...
const MEDALS = [
  { rank: "1", cartColor: "text-yellow-500", border: "ring-2 ring-yellow-400/80 shadow-md shadow-yellow-100", rowBg: "bg-gradient-to-l from-yellow-50/80 to-white", badge: "bg-yellow-400 text-white", label: "הזול ביותר", labelColor: "text-yellow-600 bg-yellow-50" },
  { rank: "2", cartColor: "text-slate-400", border: "ring-2 ring-slate-300/80 shadow-md shadow-slate-100", rowBg: "bg-gradient-to-l from-slate-50/80 to-white", badge: "bg-slate-400 text-white", label: "מקום שני", labelColor: "text-slate-500 bg-slate-50" },
  { rank: "3", cartColor: "text-amber-600", border: "ring-2 ring-amber-500/60 shadow-md shadow-amber-100", rowBg: "bg-gradient-to-l from-amber-50/60 to-white", badge: "bg-amber-600 text-white", label: "מקום שלישי", labelColor: "text-amber-700 bg-amber-50" },
];
            ...
            <div key={c.chain} className={medal ? "relative" : ""}>
              <button
                ...
                <div className="flex-1 min-w-0 text-right">
                  <div className={`font-bold text-sm ${medal ? "text-stone-900" : "text-stone-800"}`}>{c.chainHe || c.chain}</div>
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    {medal && (
                      <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-full ${medal.labelColor}`}>{medal.label}</span>
                    )}
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${color}`}>{label}</span>
                    {missingCount > 0 && (
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-red-50 text-red-500">⚠️ {missingCount} מוצרים חסרים</span>
                    )}
                  </div>
                </div>
                <div className="flex-shrink-0 text-left">
                  <div className={`font-mono font-black text-sm ${medal?.rank === "1" ? "text-yellow-600" : "text-stone-700"}`}>₪{c.totalPrice}</div>
                      ...
                      return (
                        <Link key={pid} href={`/product/${pid}`}
                          ...
                          <div className="flex-shrink-0">
                            {missing
                              ? <span className="text-xs font-bold text-red-400 bg-red-50 px-2 py-1 rounded-lg">לא נמצא</span>
