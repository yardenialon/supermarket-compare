...
function CLogo({ name, subchain, size = 40 }: { name: string; subchain?: string; size?: number }) {
  ...
  return <div className="rounded-lg flex items-center justify-center text-white font-black text-sm" style={{ width: size, height: size, background: color }}>{he.charAt(0)}</div>;
}

interface StoreResult {
  storeId: number; storeName: string; chainName: string; subchainName?: string;
  city: string; total: number; availableCount: number; missingCount: number;
    ...
    <div className="min-h-screen bg-gradient-to-b from-amber-50 via-white to-stone-50 pb-24" dir="rtl">
      <div className="relative overflow-hidden bg-gradient-to-bl from-amber-400 via-yellow-300 to-amber-200 px-4 pt-10 pb-14">
        <div className="relative max-w-2xl mx-auto text-center">
          ...
          <h1 className="text-3xl font-black text-amber-900 leading-tight mb-2">סל קניות לניקיון<br />וסדר פסח</h1>
          <p className="text-amber-800 text-sm font-medium mb-6">18 מוצרי הניקיון הנמכרים ביותר — השוואת מחירים בכל הרשתות</p>
          <div className="inline-flex bg-white/50 rounded-2xl p-1 gap-1 mb-4">
            <button onClick={() => setLocMode('cheap')} className={"px-4 py-2 rounded-xl text-sm font-bold transition-all " + (locMode === 'cheap' ? 'bg-white text-amber-800 shadow-sm' : 'text-amber-700')}>💰 הכי זול</button>
            <button onClick={() => { setLocMode('nearby'); navigator.geolocation?.getCurrentPosition(pos => { const loc = { lat: pos.coords.latitude, lng: pos.coords.longitude }; setUserLoc(loc); localStorage.setItem('savy-location', JSON.stringify(loc)); }); }} className={"px-4 py-2 rounded-xl text-sm font-bold transition-all " + (locMode === 'nearby' ? 'bg-white text-amber-800 shadow-sm' : 'text-amber-700')}>📍 הכי קרוב</button>
          </div>
          <div className="flex gap-3 justify-center">
            <button onClick={compare} disabled={loading} className="px-8 py-3.5 rounded-2xl bg-amber-900 text-white font-black text-base shadow-lg hover:bg-amber-800 transition-all disabled:opacity-60">{loading ? '⏳ משווה...' : '🔍 השווה מחירים'}</button>
            <button onClick={importToApp} className="px-5 py-3.5 rounded-2xl bg-white/70 text-amber-900 font-bold text-sm hover:bg-white transition-all">➕ הוסף לרשימה</button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 -mt-6">
        {priceDiff > 10 && (
          <div className="bg-emerald-500 text-white rounded-2xl px-5 py-4 mb-5 flex items-center justify-between shadow-lg">
            <div><div className="font-black text-lg">חסכון אפשרי: ₪{priceDiff.toFixed(0)}</div><div className="text-emerald-100 text-xs">בין הרשת הזולה ליקרה</div></div>
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {results.slice(0, 8).map((store, idx) => {
                  ...
                  return (
                    <div key={store.storeId} className={"rounded-2xl border-2 overflow-hidden transition-all " + (isCheapest ? 'border-emerald-400 bg-emerald-50/30 shadow-md' : 'border-stone-100 bg-white')}>
                      <button onClick={() => setExpanded(isOpen ? null : store.storeId)} className="w-full px-4 py-3.5 flex items-center gap-3 text-right">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-black text-stone-800 text-sm">{store.subchainName ? (SUBCHAINS[store.subchainName]?.he || store.subchainName) : (CHAINS[store.chainName]?.he || store.chainName)}</div>
                        ...
                        <div className="text-right shrink-0">
                          <div className={"text-xl font-black " + (isCheapest ? 'text-emerald-600' : 'text-stone-800')}>₪{store.total.toFixed(0)}</div>
                          {idx > 0 && cheapest && <div className="text-xs text-red-400">+₪{(store.total - cheapest.total).toFixed(0)}</div>}
                        </div>
                          ...
                          {store.breakdown.map((b, bi) => {
                            ...
                            return (
                              <div key={b.productId} className={"flex items-center justify-between py-2 text-xs " + (bi < store.breakdown.length - 1 ? 'border-b border-stone-100' : '')}>
                                <span className="text-stone-600 flex items-center gap-1.5"><span>{prod?.emoji}</span><span>{prod?.name || `מוצר #${b.productId}`}</span>{b.qty > 1 && <span className="text-stone-400">×{b.qty}</span>}</span>
                                <span className="font-bold text-stone-700">₪{b.subtotal.toFixed(2)}</span>
                              </div>
                            );
                          })}
                          <div className="flex justify-between pt-2 font-black text-sm text-stone-800 border-t border-stone-200 mt-1"><span>סה"כ</span><span>₪{store.total.toFixed(2)}</span></div>
                        </div>
                      )}
                    </div>
                  );
          ...
          <div className="grid grid-cols-1 gap-2">
            {items.map(item => (
              <div key={item.productId} className="bg-white rounded-xl px-4 py-3 border border-stone-100 flex items-center justify-between">
                <div className="flex items-center gap-3"><span className="text-2xl">{item.emoji}</span><div className="font-bold text-sm text-stone-800">{item.name}</div></div>
      ...
      <div className="fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur border-t border-stone-100 px-4 py-3 shadow-lg">
        <div className="max-w-2xl mx-auto flex gap-3">
          <button onClick={importToApp} className="flex-1 py-3 rounded-xl bg-stone-800 text-white font-bold text-sm">➕ הוסף לרשימה שלי</button>
          <button onClick={compare} disabled={loading} className="flex-1 py-3 rounded-xl bg-amber-500 text-white font-black text-sm disabled:opacity-60">{loading ? '⏳' : '🔍 השווה'}</button>
        </div>
      </div>
    </div>
  );
